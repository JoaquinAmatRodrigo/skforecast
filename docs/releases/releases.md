...
+ `metric_values` returned by <code>[backtesting_forecaster]</code> and <code>[backtesting_forecaster_sarimax]</code> is a `pandas DataFrame` with one column per metric instead of a `list`.
