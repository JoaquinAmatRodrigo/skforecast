...
def test_set_out_sample_residuals_UserWarning_when_forecaster_has_transformer_and_transform_False():
    """
    Test UserWarning is raised when forcaster has a transformer_y and transform=False.
    """
...
def test_set_out_sample_residuals_UserWarning_when_forecaster_has_transformer_and_transform_True():
    """
    Test UserWarning is raised when forcaster has a transformer_y and transform=True.
    """
    forecaster = ForecasterAutoregDirect(LinearRegression(), lags=3, steps=2, transformer_y=StandardScaler())
    forecaster.fit(y=y)
    residuals = {1: np.array([1, 2, 3]), 2: np.array([1, 2, 3])}
