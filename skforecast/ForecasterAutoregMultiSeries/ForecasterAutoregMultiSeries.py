...
# TODO: copy docstring store_last_window and store_in_sample to other methods
    def fit(
        self,
        series: Union[pd.DataFrame, dict],
        exog: Optional[Union[pd.Series, pd.DataFrame, dict]] = None,
    ...
    def predict_bootstrapping(
        ...
        for i in range(n_boot):
            ...
            for j in range(steps):
                ...
                residuals_step = (
                    np.array([sample_residuals[level][j] for level in levels])
                    .reshape(1, -1)
                )
...
# ==============================================================================
    ...
    def _recursive_predict_013(
        ...
        if self.encoding is not None:
            if self.encoding == 'onehot':
                levels_dummies = np.zeros(shape=len(self.X_train_series_names_in_), dtype=float)
                if level in self.X_train_series_names_in_:
                    levels_dummies[self.X_train_series_names_in_.index(level)] = 1.
    ...
    def predict_013(
        ...
        (
            ...
            prediction_index,
            _
        ) = self._create_predict_inputs_013(
            steps       = steps,
            levels      = levels,
            last_window = last_window,
            exog        = exog
        ...
        for level in levels:
            ...
            preds_level = self._recursive_predict_013(
                              steps       = steps,
                              level       = level,
