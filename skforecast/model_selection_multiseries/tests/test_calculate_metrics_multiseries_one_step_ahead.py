# Unit test calculate_metrics_multiseries_one_step_ahead
# ==============================================================================
import pandas as pd
import numpy as np
...
def test_calculate_metrics_multiseries_one_step_ahead_input_types():
    """
    ...
    y_pred_encoding = ['a', 'b', 'c']
    y_train = np.array([1, 2, 3])
    y_train_index = pd.DatetimeIndex(["2020-01-01", "2020-01-02", "2020-01-03"])
    y_train_encoding = ['a', 'b', 'c']
    levels = ["level1", "level2"]
    metrics = ["mean_absolute_error"]
    add_aggregated_metric = True

    # Test invalid type for y_true
    msg = "`y_true` must be a numpy array."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true='invalid',
            y_true_index=y_true_index,
            y_pred=y_pred,
            y_pred_encoding=y_pred_encoding,
            y_train=y_train,
    ...
    # Test invalid type for y_true_index
    msg = "`y_true_index` must be a pandas DatetimeIndex."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index='invalid',
            y_pred=y_pred,
            y_pred_encoding=y_pred_encoding,
            y_train=y_train,
            y_train_index=y_train_index,
    ...
    # Test invalid type for y_pred
    msg = "`y_pred` must be a numpy array."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index=y_true_index,
    ...
    # Test invalid type for y_pred_encoding
    msg = "`y_pred_encoding` must be a list."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index=y_true_index,
            y_pred=y_pred,
            y_pred_encoding='invalid',
            y_train=y_train,
            y_train_index=y_train_index,
            y_train_encoding=y_train_encoding,
            levels=levels,
    ...
    msg = "`y_train` must be a numpy array."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index=y_true_index,
    ...
    # Test invalid type for y_train_index
    msg = "`y_train_index` must be a pandas DatetimeIndex."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index=y_true_index,
    ...
    # Test invalid type for y_train_encoding
    msg = "`y_train_encoding` must be a list."
    with pytest.raises(TypeError, match=msg):
        _calculate_metrics_multiseries_one_step_ahead(
            y_true=y_true,
            y_true_index=y_true_index,
